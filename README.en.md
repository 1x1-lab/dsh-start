...
- 🔄 **Smart updates**: the "Update to vX.X.X" button only appears when the npm registry actually has a newer version. Supports a custom npm registry (e.g. npmmirror for faster mirrors) and dist-tag selection (latest / next / alpha — the dropdown shows each tag's resolved version, with a custom version input as fallback)
- 🖥️ **Console**: stat cards (port / control port / version / uptime), start / stop / restart, live logs (in-memory ring + rolling file)
- 💰 **Quota query**: reads the DSH configuration (`~/.dsh/settings.yaml` + `.credentials.yaml`, honoring `DSH_HOME`), lists every configured API automatically (DeepSeek Official + `llm-pi-ai.providers` catalog / custom gateways), and queries per API type: DeepSeek shows per-currency total / granted / topped-up balance, OpenRouter shows total credits / used / remaining, Kimi For Coding / Zhipu (z.ai, bigmodel.cn) / MiniMax show coding-plan usage (per-window percentage + progress bar + reset time), StepFun / SiliconFlow / Novita use their official endpoints, and other OpenAI-protocol endpoints fall back to the generic query (`GET {baseURL}/user/balance`). Every API has its own persistent "auto query" toggle (disabled APIs are skipped on mount and by Refresh All). Requests are issued by the Rust backend and work even when DSH isn't running. Keys are resolved only inside Rust (env vars → credentials file refs, same priority as DSH); the UI shows masked keys only — nothing written to disk or logs
...
```
src/                Vue frontend (console / wizard / logs / quota / settings / i18n)
src-tauri/
  src/
