...
4. 「额度查询」自动读取 DSH 配置列出全部 API 并逐个查询余额与额度；需先在 DSH 中为对应 API 配置好凭据（`apiKeyEnv` → `.credentials.yaml` refs 或环境变量）。每次查询实时重读配置，DSH 中改动即刻生效
...
src-tauri/
  src/
    ...
    dshconf.rs      DSH 配置读取：settings.yaml / .credentials.yaml 解析、API 列表与凭据脱敏
    quota.rs        额度查询：按 baseURL 识别供应商端点 / OpenAI 协议通用查询 / 结构化错误
...
- 「额度查询」的 API Key 只在 Rust 侧解析（环境变量 → `~/.dsh/.credentials.yaml` refs），从不回传完整 Key：界面仅显示脱敏形式，不写入 localStorage、settings.json 或日志；错误信息均不包含 Key
